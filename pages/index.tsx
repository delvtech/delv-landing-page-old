...
const useScrollDirection = (
  ...
  const isInsideSections = (element: Element | null): boolean => {
    while (element) {
      if (
        (element.classList && element.classList.contains('sections')) ||
        element.classList.contains('mobile_nav')
      ) {
        return true
      }
...
export default function Home() {
  ...
  const [footerToggles, setFooterToggles] = useState(
    Object.keys(Links).reduce((acc: Record<string, boolean>, key: string) => {
      acc[key] = false
      return acc
    }, {})
  )
  ...
  useEffect(() => {
    if (projectNavRef.current) {
      ...
      const animateScroll = (currentTime: number) => {
        ...
        projectNavRef.current.scrollTop =
  ...
  const arrowDebounce = useRef<NodeJS.Timeout | null>()

  function handleArrowClick() {
    if (arrowDebounce.current) {
      return null
    }
    if (activeSection < sections.length - 2) {
      setActiveSection(activeSection + 1)
    ...
    arrowDebounce.current = setTimeout(() => {
      arrowDebounce.current = null
    }, 600)
  }
    ...
    <>
      <Head>
        ...
        <meta
          ...
          content="DELV - The Factory for DeFi"
        />
      ...
      >
          ...
          <div className="mobile-only">
            <div
              className={
                'mobile_nav_toggle' +
                ' ' +
                (mobileMenuOpen ? 'mobile_nav_toggle__open' : '')
              }
            ...
            >
              ...
              <div className="mobile_nav_links">
                {Object.keys(Links).map((group, index) => (
                  <div key={group}>
                    <span className="mobile_nav_links_heading">{group}</span>
                ...
                <motion.div
                  ...
                  initial={{
                    opacity: Math.max(
                      isMobile ? 0.4 : 0.1,
                      0.5 -
                        Math.abs(activeSection - index) * (isMobile ? 1 : 0.2)
                    ),
                    y: 10 * (index + 4),
                    scale: 0.9,
                  }}
                  animate={{
                    ...
                    opacity: Math.max(
                      isMobile ? 0.4 : 0.1,
                      1 - Math.abs(activeSection - index) * (isMobile ? 1 : 0.2)
                    ),
        ...
        <div className={styles.content}>
          ...
          <AnimatePresence mode="wait">
            <ProjectBackground
              projectId={sections[activeSection]?.id}
              ...
              group == 'Build' ? null : ( // hotfix for socials
                <div
                  ...
                  onClick={() =>
                    setFooterToggles({
                      ...footerToggles,
                      [group]: !footerToggles[group],
                    })
                  }
                  className={
                    footerToggles[group]
                      ? styles.nav_footer_group_active
                      : styles.nav_footer_group
                  }
                >
                  <div>
                    {Links[group].map((link, index) => (
                  ...
                  <span className={styles.nav_footer_group_title}>{group}</span>
                </div>
              )
            )}
          </motion.div>
